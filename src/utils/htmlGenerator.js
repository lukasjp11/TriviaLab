...
<body>
  ...
  for (let pageStart = 0; pageStart < cards.length; pageStart += cardsPerPage) {
      ...
      categories.forEach((category, idx, arr) => {
        ...
        if (questionData && questionData.question) {
          const isLast = idx === arr.length - 1 || 
                         !arr.slice(idx + 1).some(laterCategory => 
                            card.questions.some(q => q.category === laterCategory.name && q.question)
                         );
          
          html += `
                <div class="category ${isLast ? 'no-border' : ''}">
    ...
    const isLastPage = pageEnd === cards.length;
    const isTotalCardsOdd = cards.length % 2 === 1;
    ...
    for (let i = 0; i < pageCards.length; i += 2) {
      if (i + 1 < pageCards.length) {
        ...
        if (isLastPage && isTotalCardsOdd) {
          arrangedAnswers.push(null);
          arrangedAnswers.push(pageCards[i]);
        } else {
          arrangedAnswers.push(pageCards[i]);
        }
